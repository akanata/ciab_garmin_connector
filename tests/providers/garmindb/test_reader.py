...
def reader_for(settings: Settings, **kwargs: object) -> GarminDbReader:
    build_fixture(settings.health_data_dir, **kwargs)  # type: ignore[arg-type]
    return GarminDbReader(GarminConnection(settings))


def test_it_satisfies_the_reader_port(corpus_settings: Settings) -> None:
    reader = reader_for(corpus_settings, nights=1, heart_rate=True)
    assert isinstance(reader, HealthReader)


def test_it_offers_the_catalog_bound_to_its_own_corpus(corpus_settings: Settings) -> None:
    reader = reader_for(corpus_settings, nights=1, heart_rate=True)
    assert {k for k, e in reader.metrics.items() if e.probe()} == {"heart_rate"}


def test_it_builds_sessions_newest_first(corpus_settings: Settings) -> None:
...
def test_the_session_limit_bounds_the_work(corpus_settings: Settings) -> None:
    reader = reader_for(corpus_settings, nights=3)
    assert len(reader.sleep_sessions(None, None, 1)) == 1


def test_a_healthy_corpus_has_no_fault(corpus_settings: Settings) -> None:
    assert reader_for(corpus_settings, nights=1, heart_rate=True).fault is None


def test_an_empty_corpus_reports_holding_nothing(corpus_settings: Settings) -> None:
    assert reader_for(corpus_settings, nights=0).has_any_data() is False


def test_a_populated_corpus_reports_holding_something(corpus_settings: Settings) -> None:
    assert reader_for(corpus_settings, nights=1, heart_rate=True).has_any_data() is True


def test_a_stale_schema_surfaces_as_a_fault(corpus_settings: Settings) -> None:
...
def test_closing_it_closes_the_corpus(corpus_settings: Settings) -> None:
    reader = reader_for(corpus_settings, nights=1, heart_rate=True)
    reader.close()
