...
from garmin_health.providers.garmindb.sync import SyncPhase
...
def linked_authenticator(settings: GarminDbSettings) -> GarminAuthenticator:
...
def make_engine(
    ...
    *,
    ...
    engine = SyncEngine(
        settings=GarminDbSettings(
            app_data_dir=settings.app_data_dir, sync_interval_seconds=interval
        ),
        authenticator=auth,
...
class TestRunOnce:
    async def test_runs_the_phases_in_the_required_order(self, settings: GarminDbSettings) -> None:
    ...
    async def test_records_a_duration(self, settings: GarminDbSettings) -> None:
    ...
    async def test_reports_row_growth(self, settings: GarminDbSettings) -> None:
        ingest = FakeIngest(
            stats_sequence=[
    ...
    async def test_surfaces_a_sync_that_imported_nothing(self, settings: GarminDbSettings) -> None:
    ...
    async def test_a_failing_phase_is_recorded_not_raised(self, settings: GarminDbSettings) -> None:
    ...
    async def test_a_later_phase_failure_still_names_that_phase(
    ...
    async def test_refuses_to_run_when_the_account_is_not_linked(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_does_not_run_on_the_event_loop(self, settings: GarminDbSettings) -> None:
...
class TestOverlap:
    ...
    async def test_the_lock_is_released_after_a_failure(self, settings: GarminDbSettings) -> None:
...
class TestTrigger:
    ...
    async def test_trigger_is_refused_while_running(self, settings: GarminDbSettings) -> None:
        release = Event()
...
class TestStatus:
    ...
    async def test_reports_the_last_error(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings, ingest=FakeIngest(fail_on="download"))
        await engine.run_once()
        last = engine.status()["last_sync"]
        assert last is not None
    ...
    async def test_timestamps_are_aware_utc(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings)
        report = await engine.run_once()
        assert report is not None
...
def plant_last_start(settings: GarminDbSettings, when: dt.datetime) -> None:
...
class TestSchedule:
    ...
    async def test_a_never_synced_container_waits_only_the_grace(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_the_start_is_recorded_before_any_garmin_work(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_the_recorded_start_carries_into_a_new_process(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_a_backfill_does_not_move_the_schedule(self, settings: GarminDbSettings) -> None:
    ...
    async def test_an_interval_change_does_not_wait_out_the_old_one(
        ...
        ingest = FakeIngest()
        engine = SyncEngine(
            settings=GarminDbSettings(app_data_dir=settings.app_data_dir),
    ...
    async def test_status_reports_the_interval_in_force(self, settings: GarminDbSettings) -> None:
        engine = SyncEngine(
            settings=GarminDbSettings(app_data_dir=settings.app_data_dir),
            authenticator=linked_authenticator(settings),
    ...
    async def test_a_corrupt_state_file_counts_as_never_synced(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_it_keeps_syncing_once_per_interval(self, settings: GarminDbSettings) -> None:
        engine, ingest, _ = make_engine(settings, interval=0)
        await run_loop_for(engine, 0.1)
        assert ingest.calls.count("download") >= 2

    async def test_it_does_nothing_while_unlinked(self, settings: GarminDbSettings) -> None:
        engine, ingest, _ = make_engine(settings, linked=False, interval=0)
        await run_loop_for(engine, 0.05)
        assert ingest.calls == []

    async def test_a_failing_sync_does_not_kill_the_loop(self, settings: GarminDbSettings) -> None:
...
class TestCancellation:
    async def test_requesting_stop_signals_the_running_ingest(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_a_fresh_run_clears_a_previous_stop_request(
        self, settings: GarminDbSettings
    ) -> None:
        engine, ingest, _ = make_engine(settings)
        engine.request_stop()
...
class TestResilience:
    ...
    async def test_the_loop_survives_an_unexpected_engine_error(
        self, settings: GarminDbSettings
    ) -> None:
...
class TestCorpusChangedCallback:
    ...
    async def test_it_fires_after_a_successful_sync(self, settings: GarminDbSettings) -> None:
        fired: list[int] = []
        engine = SyncEngine(
            settings=settings,
            authenticator=linked_authenticator(settings),
    ...
    async def test_it_fires_after_a_failed_sync_too(self, settings: GarminDbSettings) -> None:
    ...
    async def test_a_failing_callback_does_not_fail_the_sync(
        self, settings: GarminDbSettings
    ) -> None:
        def explode() -> None:
    ...
    async def test_it_does_not_fire_when_no_sync_ran(self, settings: GarminDbSettings) -> None:
        fired: list[int] = []
        engine = SyncEngine(
            settings=settings,
...
class TestRebuild:
    async def test_it_runs_the_rebuild_and_nothing_else(self, settings: GarminDbSettings) -> None:
    ...
    async def test_it_reports_a_failure_rather_than_raising(
        self, settings: GarminDbSettings
    ) -> None:
        engine = SyncEngine(
            settings=settings,
    ...
    async def test_it_resets_the_serving_layer_afterwards(self, settings: GarminDbSettings) -> None:
    ...
    async def test_it_needs_no_linked_account(self, settings: GarminDbSettings) -> None:
    ...
    async def test_it_refuses_to_start_while_a_sync_is_in_flight(
        self, settings: GarminDbSettings
    ) -> None:
        engine = SyncEngine(
            settings=settings,
...
class TestProgress:
    async def test_no_progress_is_reported_while_idle(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings)
        assert engine.status()["progress"] is None

    async def test_the_engine_reports_the_step_the_ingest_is_on(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    async def test_the_step_is_cleared_when_the_sync_ends(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings)
        await engine.run_once()
        assert engine.status()["progress"] is None

    async def test_the_step_is_cleared_after_a_failure_too(
        self, settings: GarminDbSettings
    ) -> None:
...
class TestBackfill:
    async def test_it_downloads_the_older_range_then_imports(
        self, settings: GarminDbSettings
    ) -> None:
        ingest = FakeIngest()
        engine = SyncEngine(
    ...
    async def test_a_normal_sync_still_downloads_forward(self, settings: GarminDbSettings) -> None:
        engine, ingest, _ = make_engine(settings)
        await engine.run_once()
        assert "download" in ingest.calls
        assert "backfill" not in ingest.calls

    async def test_it_needs_a_linked_account(self, settings: GarminDbSettings) -> None:
        """Unlike a rebuild, this really does talk to Garmin."""
        engine, _, _ = make_engine(settings, linked=False)
        assert await engine.run_once(backfill=True) is None

    async def test_triggering_it_reports_started(self, settings: GarminDbSettings) -> None:
        engine, ingest, _ = make_engine(settings)
        assert await engine.trigger(backfill=True) is True
        await engine.wait_for_idle()
        assert "backfill" in ingest.calls

    async def test_it_will_not_start_alongside_a_sync(self, settings: GarminDbSettings) -> None:
...
class TestCoverageReporting:
    async def test_it_exposes_what_each_metric_holds(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings)
        coverage = await engine.coverage()
        assert "sleep" in coverage
        assert coverage["sleep"].rows == 42

    async def test_it_is_cached_between_page_loads(self, settings: GarminDbSettings) -> None:
    ...
    async def test_a_sync_invalidates_the_cache(self, settings: GarminDbSettings) -> None:
        engine, _, _ = make_engine(settings)
        await engine.coverage()
        await engine.run_once()
