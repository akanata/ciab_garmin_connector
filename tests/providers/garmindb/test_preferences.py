...
class TestDefaults:
    def test_the_env_var_is_the_default_start_date(self, settings: GarminDbSettings) -> None:
    ...
    def test_an_overridden_env_var_is_honoured(self, tmp_path: Path) -> None:
        settings = GarminDbSettings(app_data_dir=tmp_path / "a", backfill_start_date="2025-01-01")
        assert load_preferences(settings).start_date == dt.date(2025, 1, 1)

    def test_every_downloadable_stat_is_on_by_default(self, settings: GarminDbSettings) -> None:
...
class TestRoundTrip:
    def test_saved_preferences_are_read_back(self, settings: GarminDbSettings) -> None:
    ...
    def test_the_file_lands_under_app_data(self, settings: GarminDbSettings) -> None:
    ...
    def test_it_is_written_atomically(self, settings: GarminDbSettings) -> None:
        save_preferences(settings, load_preferences(settings))
        assert list(settings.app_data_dir.glob("*.tmp")) == []

    def test_the_stored_document_is_readable_json(self, settings: GarminDbSettings) -> None:
        save_preferences(
            settings,
...
class TestCorruptFile:
    def test_unparseable_json_falls_back_to_the_defaults(
        self, settings: GarminDbSettings, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_an_unknown_stat_in_the_file_is_dropped(self, settings: GarminDbSettings) -> None:
    ...
    def test_a_bad_date_in_the_file_falls_back_to_the_default(
...
class TestParsingFormInput:
    def test_it_accepts_what_a_date_input_submits(self, settings: GarminDbSettings) -> None:
    ...
    def test_an_empty_selection_means_pause_imports(self, settings: GarminDbSettings) -> None:
    ...
    def test_an_unparseable_date_is_refused(self, settings: GarminDbSettings, bad: str) -> None:
    ...
    def test_today_itself_is_allowed(self, settings: GarminDbSettings) -> None:
        assert (
            parse_preferences(
...
def test_preferences_render_the_garmindb_date_format(settings: GarminDbSettings) -> None:
...
class TestSyncInterval:
    ...
    def test_the_env_var_seeds_the_default(self, tmp_path: Path) -> None:
        settings = GarminDbSettings(app_data_dir=tmp_path / "a", sync_interval_seconds=1800)
        assert load_preferences(settings).sync_interval_seconds == 1800

    def test_it_round_trips(self, settings: GarminDbSettings) -> None:
        prefs = ImportPreferences(
    ...
    def test_an_unusable_stored_interval_falls_back_to_the_default(
        self, settings: GarminDbSettings, bad: object
    ) -> None:
        settings.preferences_file.parent.mkdir(parents=True, exist_ok=True)
        settings.preferences_file.write_text(
            json.dumps(
    ...
    def test_every_offered_choice_is_accepted(self, settings: GarminDbSettings) -> None:
    ...
    def test_an_interval_that_is_not_offered_is_refused(
        self, settings: GarminDbSettings, bad: str
    ) -> None:
    ...
    def test_the_operators_env_value_is_accepted_even_if_not_offered(self, tmp_path: Path) -> None:
        ...
        settings = GarminDbSettings(app_data_dir=tmp_path / "a", sync_interval_seconds=7200)
        prefs = parse_preferences(
            settings, start_date="2024-03-01", stats=["sleep"], sync_interval="7200", today=TODAY
        )
        assert prefs.sync_interval_seconds == 7200

    def test_omitting_the_interval_keeps_the_saved_one(self, settings: GarminDbSettings) -> None:
        save_preferences(
            settings,
            ImportPreferences(
                start_date=dt.date(2024, 3, 1),
