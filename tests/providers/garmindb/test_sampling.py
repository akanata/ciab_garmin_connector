...
class TestColumnSeries:
    def test_it_emits_aware_utc_samples_of_the_cast_type(
    ...
    def test_the_window_is_half_open_on_the_real_instants(
        self, corpus_settings: GarminDbSettings
    ) -> None:
    ...
    def test_it_decimates_to_the_requested_limit(self, corpus_settings: GarminDbSettings) -> None:
        build_fixture(corpus_settings.health_data_dir, nights=1, heart_rate=True)
        build = column_series(MonitoringHeartRate, MonitoringHeartRate.heart_rate, db="monitoring")
        with GarminConnection(corpus_settings) as conn:
    ...
    def test_an_empty_window_is_an_empty_list_not_an_error(
        self, corpus_settings: GarminDbSettings
    ) -> None:
    ...
    def test_it_reads_the_garmin_database_when_told_to(
        self, corpus_settings: GarminDbSettings
    ) -> None:
...
class TestHasRows:
    def test_it_reports_a_populated_table(self, corpus_settings: GarminDbSettings) -> None:
        build_fixture(corpus_settings.health_data_dir, nights=1, heart_rate=True)
        probe = has_rows(MonitoringHeartRate, MonitoringHeartRate.heart_rate, db="monitoring")
        with GarminConnection(corpus_settings) as conn:
            assert probe(conn) is True

    def test_it_reports_an_empty_table(self, corpus_settings: GarminDbSettings) -> None:
        build_fixture(corpus_settings.health_data_dir, nights=1, heart_rate=True)
        probe = has_rows(MonitoringHrvValue, MonitoringHrvValue.hrv, db="monitoring")
        with GarminConnection(corpus_settings) as conn:
            assert probe(conn) is False

    def test_a_table_of_only_nulls_counts_as_empty(self, corpus_settings: GarminDbSettings) -> None:
