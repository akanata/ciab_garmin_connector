"""GarminDB's half of the owner page: linking, import scope, coverage, syncing.

The shell around this -- the guard, the headline per link state, the one-shot
flash, unlink -- is pinned provider-free in ``tests/test_owner_routes.py``, and
that every one of these handlers is mounted behind the owner guard is pinned for
all providers at once in ``tests/providers/test_contract.py``. What is here is
everything that only means something when the data is acquired by driving
GarminDB.
"""
...
from garmin_health.providers.garmindb.owner import MINT_SNIPPET
from garmin_health.providers.garmindb.owner import _join_names
...
def client(settings: GarminDbSettings, factory: RecordingFactory) -> Iterator[TestClient]:
    yield from client_for(make_provider(settings, factory=factory))


def unlinked_client(settings: GarminDbSettings, **kwargs: object) -> Iterator[TestClient]:
    yield from client_for(make_provider(settings, **kwargs))  # type: ignore[arg-type]
...
def linked(
    ...
    provider = make_provider(settings, ingest=ingest, linked=True)
    return TestClient(app=create_app(provider=provider)), ingest
...
class TestSignIn:
    def test_the_page_names_garmin_connect(self, client: TestClient) -> None:
        response = client.get("/setup", headers=OWNER)
        assert response.status_code == 200
        assert "text/html" in response.headers["content-type"]
        assert "Garmin Connect" in response.text

    def test_it_warns_that_the_password_is_replayed(self, client: TestClient) -> None:
        """There is no OAuth consent dialog for this API. The owner is handing over
        a real Garmin password and the page has to say so."""
        assert "password" in client.get("/setup", headers=OWNER).text.lower()

    def test_credentials_post_links_the_account(self, client: TestClient) -> None:
    ...
    def test_the_mfa_flow_moves_through_pending_to_linked(self, settings: GarminDbSettings) -> None:
        factory = RecordingFactory(needs_mfa=True)
        for client in client_for(make_provider(settings, factory=factory)):
            client.post(
                "/setup/credentials",
                data={"email": "rider@example.com", "password": "hunter2"},
                headers=OWNER,
            )
            assert client.get("/setup/status", headers=OWNER).json()["state"] == "pending"
    ...
    def test_missing_form_fields_are_rejected_cleanly(self, client: TestClient) -> None:
        response = client.post(
    ...
    def test_unlink_returns_to_not_linked(self, client: TestClient) -> None:
        client.post(
            "/setup/credentials",
            data={"email": "rider@example.com", "password": "hunter2"},
            headers=OWNER,
    ...
    def test_the_password_is_never_echoed_back_into_the_page(self, client: TestClient) -> None:
        client.post(
            "/setup/credentials",
            data={"email": "rider@example.com", "password": "hunter2"},
            headers=OWNER,
    ...
    def test_the_email_is_escaped(self, client: TestClient) -> None:
        """The email is owner-supplied and lands in HTML; it must not be able to
        close an attribute and inject markup."""
        client.post(
            "/setup/credentials",
    ...
    def test_the_form_still_posts_without_javascript(self, client: TestClient) -> None:
...
class TestSignInFeedback:
    def test_a_failed_sign_in_message_does_not_survive_a_refresh(
        self, settings: GarminDbSettings
    ) -> None:
        """Regression: the failure was stored on the authenticator and re-rendered
        on every GET, so /setup kept reporting a sign-in failure indefinitely."""
        factory = RecordingFactory(login_error=GarminConnectAuthenticationError("bad password"))
        for client in client_for(make_provider(settings, factory=factory)):
            first = client.post(
                "/setup/credentials",
    ...
    def test_a_failed_mfa_message_does_not_survive_a_refresh(
        self, settings: GarminDbSettings
    ) -> None:
    ...
    def test_the_status_endpoint_reports_the_error_without_consuming_it(
        self, settings: GarminDbSettings
    ) -> None:
        factory = RecordingFactory(login_error=GarminConnectAuthenticationError("bad password"))
        for client in client_for(make_provider(settings, factory=factory)):
    ...
    def test_the_pending_prompt_is_not_a_flash(self, settings: GarminDbSettings) -> None:
    ...
    def test_the_sign_in_form_declares_an_in_flight_message(self, client: TestClient) -> None:
        """A real Garmin sign-in is a multi-second SSO round-trip; the page has to
        say something is happening or the owner assumes the button did nothing."""
        page = client.get("/setup", headers=OWNER).text
        assert "data-busy=" in page
        assert "Contacting Garmin" in page
        assert "spinner" in page

    def test_the_mfa_form_declares_an_in_flight_message(self, settings: GarminDbSettings) -> None:
        factory = RecordingFactory(needs_mfa=True)
        for client in client_for(make_provider(settings, factory=factory)):
            client.post(
                "/setup/credentials",
...
class TestSyncEndpoints:
    ...
    def test_status_reports_where_the_corpus_is(self, settings: GarminDbSettings) -> None:
        """Provider-specific, so it sits under its own key rather than in the
        generic serving block."""
        client, _ = linked(settings)
        with client:
            corpus = client.get("/sync/status", headers=OWNER).json()["corpus"]
        assert corpus["db_dir"] == str(settings.db_dir)
        assert "timezone" in corpus

    def test_trigger_accepts_and_reports_that_it_started(self, settings: GarminDbSettings) -> None:
        client, ingest = linked(settings)
        with client:
    ...
    def test_trigger_is_refused_while_the_account_is_unlinked(
        ...
        for client in unlinked_client(settings):
            response = client.post("/sync", headers=OWNER)
    ...
    def test_status_surfaces_a_failed_sync(self, settings: GarminDbSettings) -> None:
        client, _ = linked(settings, FakeIngest(fail_on="download"))
        with client:
    ...
    def test_status_surfaces_a_sync_that_changed_nothing(self, settings: GarminDbSettings) -> None:
        ...
        client, _ = linked(settings, FakeIngest(stats_sequence=[same, same]))
        with client:
            client.post("/sync", headers=OWNER)
            last = wait_for_sync(client)["last_sync"]
...
class TestImportScopeControls:
    ...
    def test_the_rejected_form_is_re_rendered_inside_the_whole_page(
        self, settings: GarminDbSettings
    ) -> None:
        """The 400 goes back through the shell, so the explanation arrives attached
        to the form that caused it rather than on a bare error page."""
        client, _ = linked(settings)
        with client:
            response = client.post(
                "/setup/import",
        ...
        assert "Linked to Garmin Connect" in response.text
...
class TestCoverageDisplay:
    ...
    def test_a_complete_metric_offers_no_backfill(self, settings: GarminDbSettings) -> None:
        client, _ = linked(settings, FakeIngest(coverage_gap=False))
        with client:
            page = client.get("/setup", headers=OWNER).text
...
class TestBackfillTrigger:
    ...
    def test_it_is_refused_while_unlinked(self, settings: GarminDbSettings) -> None:
        ...
        for client in unlinked_client(settings):
            assert client.post("/backfill", headers=OWNER).status_code == 409


class TestProgressDisplay:
    def test_the_page_shows_the_step_a_running_sync_is_on(self, settings: GarminDbSettings) -> None:
        gate = threading.Event()
        client, _ = linked(settings, FakeIngest(block_on=gate))
        try:
            with client:
                client.post("/sync", headers=OWNER)
                for _ in range(100):
...
class TestCoverageWording:
    ...
    def test_a_metric_holding_nothing_is_not_called_complete(
        ...
        client, _ = linked(settings, FakeIngest(empty_metrics=True))
        with client:
            page = client.get("/setup", headers=OWNER).text
    ...
    def test_a_paused_metric_that_holds_data_says_paused(self, settings: GarminDbSettings) -> None:
        ...
        client, _ = linked(settings, FakeIngest(disabled_metrics=True))
        with client:
            page = client.get("/setup", headers=OWNER).text
...
class TestTokenImportPage:
    ...
    def test_a_linked_account_is_not_offered_it(self, settings: GarminDbSettings) -> None:
        ...
        client, _ = linked(settings)
        with client:
            assert "action='/setup/token'" not in client.get("/setup", headers=OWNER).text
