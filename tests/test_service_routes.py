...
class TestMetrics:
    ...
    def test_it_is_not_owner_gated(self, client: TestClient) -> None:
        """/v1/* arrives through the router's internal service proxy, which stamps
...
class TestTimeSeries:
    def test_the_body_is_a_bare_time_series_with_no_envelope(self, client: TestClient) -> None:
        ...
        payload = client.get("/v1/time-series", params={"metric": "heart_rate"}).json()
    ...
    def test_it_round_trips_through_the_consumers_converter(
        ...
        response = client.get("/v1/time-series", params={"metric": "heart_rate", "limit": 10})
    ...
    def test_an_unknown_metric_is_404(self, client: TestClient) -> None:
        assert client.get("/v1/time-series", params={"metric": "nope"}).status_code == 404
    ...
    def test_a_missing_metric_parameter_is_400(self, client: TestClient) -> None:
        assert client.get("/v1/time-series").status_code == 400

    def test_an_unparseable_bound_is_400(self, client: TestClient) -> None:
        response = client.get(
            "/v1/time-series", params={"metric": "heart_rate", "start": "last tuesday"}
        )
        assert response.status_code == 400

    @pytest.mark.parametrize("limit", ["0", "-5"])
    def test_a_non_positive_limit_is_400(self, client: TestClient, limit: str) -> None:
        response = client.get("/v1/time-series", params={"metric": "heart_rate", "limit": limit})
        assert response.status_code == 400
...
class TestSleepSessions:
    ...
    def test_sessions_round_trip_with_their_stage_intervals_intact(
        ...
        payload = client.get("/v1/sleep-sessions").json()
    ...
    def test_the_limit_is_honoured(self, client: TestClient) -> None:
        payload = client.get("/v1/sleep-sessions", params={"limit": 1}).json()
        assert len(payload["data"]) == 1

    def test_the_window_is_honoured(self, client: TestClient, corpus: Fixture) -> None:
        payload = client.get(
            "/v1/sleep-sessions", params={"start": corpus.newest.start_utc.isoformat()}
        ).json()
        assert len(payload["data"]) == 1

    def test_an_unparseable_bound_is_400(self, client: TestClient) -> None:
        assert client.get("/v1/sleep-sessions", params={"start": "soon"}).status_code == 400
...
class TestWorkouts:
    ...
    def test_a_single_workout_is_404(self, client: TestClient) -> None:
        ...
        assert client.get("/v1/workouts/anything").status_code == 404
...
class TestDegraded:
    ...
    def test_time_series_reports_503(self, broken_client: TestClient) -> None:
        assert (
            broken_client.get("/v1/time-series", params={"metric": "heart_rate"}).status_code == 503
        )

    def test_sleep_sessions_report_503(self, broken_client: TestClient) -> None:
        assert broken_client.get("/v1/sleep-sessions").status_code == 503

    def test_the_catalog_is_empty_rather_than_failing(self, broken_client: TestClient) -> None:
        assert broken_client.get("/v1/metrics").json() == {"metrics": []}
...
class TestNeverSynced:
    ...
    def test_every_endpoint_answers_emptily(self, fresh_client: TestClient) -> None:
        ...
        assert fresh_client.get("/v1/metrics").json() == {"metrics": []}
        assert fresh_client.get("/v1/sleep-sessions").json() == {"data": []}
        series = fresh_client.get("/v1/time-series", params={"metric": "heart_rate"})
...
class TestRebuildFromTheOwnerPage:
    ...
    def test_rebuild_clears_the_fault_and_restores_service(
        ...
        assert broken.get("/v1/sleep-sessions").status_code == 503
        ...
        assert broken.get("/v1/sleep-sessions").status_code == 200
